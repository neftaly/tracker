...
function vehicleTable_Create(id)
{
	...
	if (v.team != 0 && v.mapImage != "")
		cell.appendChild(VehicleMapIconList[v.mapImage][v.team - 1].cloneNode(false))

	// Try to find vehicle name in dictionary
	if (v.name in vehicleData)
		cell.innerHTML += vehicleData[v.name].HudName
	else
		cell.innerHTML += v.name
...
function vehicleTable_AddPlayer(table, vehicle, player, rownum)
{
	...
	row.insertCell(0).innerHTML = seatName
