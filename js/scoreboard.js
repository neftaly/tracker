...
function playerRow_Create(id)
{
	...
	row.cells[0].innerHTML = AllPlayers[id].name
...
function playerRow_Update(id)
{
	...
	row.cells[1].innerHTML = p.kills + "/" + p.deaths

	row.cells[2].innerHTML = ""
	if (p.kit != "")
		row.cells[2].appendChild(KitIconList[p.kitImage].cloneNode(false))
	...
	if (p.vehicleid >= 0 && (p.vehicleid in AllVehicles))
	{
		...
		if (vobj.menuImage != "")
			row.cells[3].appendChild(VehicleMenuIconList[vobj.menuImage].cloneNode(false))
	}
}
...
// i - player id
// del - does it need to be looked up and removed from the scoreboard (saves time if we're going to rewrite everything when loading states)
